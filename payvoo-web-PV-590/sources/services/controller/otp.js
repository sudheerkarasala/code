...
const generateOTP = function (request, response) {
	...
	//	return response.send(ResponseHelper.buildFailureResponse(configVariable.message.checkEmailMobile));
	...
	otp.isValidOTP(otpRequestData.otpValue, otpRequestData.otpId)
		.then(result => {
			...
			if (new Date(result[0].created).getTime() + 5 * 60 * 1000 < new Date().getTime()) {
				...
				//return response.send(ResponseHelper.buildSuccessResponse({},configVariable.message.otpExpire));
				return response.send({ "message": configVariable.message.otpExpire, status: STATUS.SUCCESS, expire: EXPIRE.SUCCESS });
			} else {
				logger.info("otp verified ");
			//	return response.send(ResponseHelper.buildSuccessResponse({},configVariable.message.otpVerifiedTrue));
				return response.send({ "message": configVariable.message.otpVerifiedTrue, status: STATUS.SUCCESS });
			}
		})
		.catch(err => {
		//	return response.send(ResponseHelper.buildFailureResponse(configVariable.message.otpFailed));
